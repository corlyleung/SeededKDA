...
int *LoadIndices(string sFilePath, int *iNoTokens=NULL, int nTokens=0){
	...
	if(fin.fail())
            cerr << "Unable to open file 30 " << sFilePath << " to load indices" << endl;    
	else{
		while(! fin.eof()){
			string s;
			getline(fin,s);
...
Pair<int *,int *> LoadTestData(CLLDAConfig ldaConfig, int &iTestTokens, int &W, int &D){
    ...
    if(! ldaConfig.m_sSrcWordTestIndicesPath.empty()){
    	cerr << "Unable to open file 30 src" << endl;
        ...
        cerr << "Unable to open file 30 src1" << endl;
    ...
    if(! ldaConfig.m_sTgtWordTestIndicesPath.empty()){
    	cerr << "Unable to open file 30 tgt" << endl;
        ...
        cerr << "Unable to open file 30 tgt1" << endl;
